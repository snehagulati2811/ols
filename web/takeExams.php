...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $.ajax({
            ...
            url: 'validateUserRegistration.php',
            type: 'POST',
            data: dataset,
            dataType: 'json',
                    ...
                    if ( parseInt(regcount) > 0 ) {
                        ...
                        $.ajax({
                            ...
                            success:function(response) {
                                ...
                                $("#tGrid").empty();
                                $("#tGrid").append("<thead><tr><th>S.No</th><th>Course</th><th>Correct Answers</th><th>Action</th></tr></thead><tbody>");
                                for ( var i = 0; i <len; i++ ) {
                                        var cid = response[i]['cid'],
                                        course = response[i]['course'],
                                        attempt = response[i]['attempt'],
                                        score = response[i]['score'],
                                        correctAnswer = response[i]['correctAnswer'],
                                        totalQuestion = response[i]['totalQuestion'];

                                        var outof = 10 * parseInt(totalQuestion);
                                        if ( parseInt(attempt) >= 1 ) {
                                            $("#tGrid").append("<tr><td>"+(i+1)+"</td><td>"+course+"</td><td>"+correctAnswer+" out of "+totalQuestion+"</td><td>Completed, your score is,"+score+"/"+outof+"</td></tr>");
                                        } else {
                                            $("#tGrid").append("<tr><td>"+(i+1)+"</td><td>"+course+"</td><td></td><td><input type='button' id='"+cid+"' onclick='takeExam("+cid+")' value='Take Exam'/></td></tr>");
                                        }
                                }
                                $("#tGrid").append("</tbody>"); 
                            }
                        ...
                        $("#tGrid").empty();
                        $("#tGrid").append("<thead><tr><th>Registered users, Please contact adminstrator for course registration.</th></tr></thead><tbody>");
                    }
            }
        });
    ...
    function takeExam(cidval) {
        console.log('Inside takeExam : ', cidval);
        
        var uidval = getCookie('uid');
        window.location = '/OLS/web/exam.php?q='+cidval;
    }
